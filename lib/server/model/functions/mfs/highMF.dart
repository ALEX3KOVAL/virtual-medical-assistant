...
class HighMembershipFunction implements IMembershipFunction {
  List<double> __coeffs = [];

  HighMembershipFunction(List<double> coeffs) {
    __coeffs = coeffs;
  }
  ...
  double __derivativeByMu(double x) =>
    x <= __coeffs[0] ? (((2 * __coeffs[2]) / __coeffs[1]) * pow(((x - __coeffs[0]) / __coeffs[1]), 2 * __coeffs[2] - 1)) /  pow(__denominator(x), 2) : 0.0;

  double __derivativeBySigma(double x) =>
    x <= __coeffs[0] ? (((2 * __coeffs[2]) / __coeffs[1]) * pow(((x - __coeffs[0]) / __coeffs[1]), 2 * __coeffs[2])) / pow(__denominator(x), 2) : 0.0;

  double __derivativeByPowCoeff(double x) =>
    x <= __coeffs[0] ? (-2.0 * pow(((x - __coeffs[0]) / __coeffs[1]), 2 * __coeffs[2]) * log((x - __coeffs[0]) / __coeffs[1])) / pow(__denominator(x), 2) : 0.0;
